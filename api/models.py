...
class CourseSubCategory(models.Model):
    ...
    category=models.ForeignKey(to='CourseCategory',on_delete="name")
    name=models.CharField(max_length=64,unique=True)
...
class Scholarship(models.Model):
    """学位课程奖学金"""
    degree_course = models.ForeignKey("DegreeCourse","name")
...
class Course(models.Model):
    ...
    sub_category=models.ForeignKey(to='CourseSubCategory',on_delete="name")
    course_type_choices=((0,'付费'),(1,'VIP专享'),(2,'学位课程'))
    course_type=models.SmallIntegerField(choices=course_type_choices)
    degree_course=models.ForeignKey(to='DegreeCourse',on_delete="name",blank=True, null=True, help_text="若是学位课程，此处关联学位表")
...
class CourseDetail(models.Model):
    """课程详情页内容"""
    course = models.OneToOneField("Course",on_delete="name")
...
class OftenAskedQuestion(models.Model):
    """常见问题"""
    content_type = models.ForeignKey(ContentType,on_delete="name")  # 关联course or degree_course
...
class CourseOutline(models.Model):
    """课程大纲"""
    course_detail = models.ForeignKey("CourseDetail","name")
...
class CourseChapter(models.Model):
    """课程章节"""
    course = models.ForeignKey("Course", on_delete="name",related_name='coursechapters')
...
class CourseSection(models.Model):
    """课时目录"""
    chapter = models.ForeignKey("CourseChapter", on_delete="name",related_name='coursesections')
    name = models.CharField(max_length=128)
...
class Homework(models.Model):
    chapter = models.ForeignKey("CourseChapter",on_delete="name")
...
class PricePolicy(models.Model):
    """价格与有课程效期表"""
    content_type = models.ForeignKey(ContentType,on_delete="name",verbose_name="价格关联的表名称")  # 关联course or degree_course
...
class UserToken(models.Model):
    username = models.ForeignKey(to='UserInfo',on_delete="name")
...
class Article(models.Model):
    ...
    source = models.ForeignKey("ArticleSource", on_delete="name",verbose_name="来源")
...
class Collection(models.Model):
    """收藏"""
    content_type = models.ForeignKey(ContentType,on_delete="name")
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')

    account = models.ForeignKey("UserInfo",on_delete="name")
...
class Comment(models.Model):
    """通用的评论表"""
    content_type = models.ForeignKey(ContentType,on_delete="name", blank=True, null=True, verbose_name="类型")
    object_id = models.PositiveIntegerField(blank=True, null=True)
    content_object = GenericForeignKey('content_type', 'object_id')

    p_node = models.ForeignKey("self",on_delete="name", blank=True, null=True, verbose_name="父级评论")
    content = models.TextField(max_length=1024)
    account = models.ForeignKey("UserInfo",on_delete="name", verbose_name="会员名")
